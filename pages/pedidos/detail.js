...
import { ACTUALIZAR_PEDIDO, ELIMINAR_PEDIDO, OBTENER_PEDIDOS} from './type';
import { useMutation } from '@apollo/client';
import Swal from 'sweetalert2';

export default function Detail({pedido}) {
    ...
    return (
        <div key={id} className={` ${clase} border-t-4 mt-4 bg-white rounded p-6 md:grid md:grid-cols-2 md:gap-4 shadow-lg`}>
            <div>
                ...
                <select
                    className="mt-2 appearance-none bg-blue-600 border border-blue-600 text-white p-2 text-center rounded leading-tight focus:outline-none focus:bg-blue-600 focus:border-blue-500 uppercase text-xs font-bold cursor-pointer"
                    value={estadoPedido}
            ...
            <div>
                ...
                { pedido.pedido.map( articulo => (
                    <div key={articulo.id} className="mt-4">
                        <p className="text-sm text-gray-600">Producto: {!articulo.producto ? '' : articulo.producto.nombre} </p>
